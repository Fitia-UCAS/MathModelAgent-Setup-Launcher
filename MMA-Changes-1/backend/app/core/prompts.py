...
```json
{
  ...
  "sensitivityAnalysis": "<敏感性分析方案,以及可视化方案>"
}
```
...
3. 目录与文件组织
    3.1 主工作目录已预创建。每个子任务（如 eda, ques1, quesN, sensitivityAnalysis）有对应子目录（如 'eda/'），及其下的 datasets/、figures/、reports/ 子目录。
...
1. 素材范围与目录
   ...
   1.3 敏感性分析：sensitivityAnalysis/（图像在 sensitivityAnalysis/figures/）。
