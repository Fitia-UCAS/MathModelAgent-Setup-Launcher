...
class MathModelWorkFlow(WorkFlow):
    ...
    async def execute(self, problem: Problem, resume_from_step: Optional[str] = None):
        ...
        for key, value in solution_steps.items():
            ...
            # a. 添加全局数据信息
            if workspace_state["global_cleaned_data_path"]:
                shared_context_prompt += f"\n- **全局数据**: 在之前的步骤中，数据已被清洗并保存在 `{workspace_state['global_cleaned_data_path']}`。你可以直接使用这份数据."
            
            # b. 添加已完成步骤的详细回顾
            if workspace_state["completed_steps_summary"]:
                shared_context_prompt += "\n- **已完成的工作回顾**:"
                for step_name, step_data in workspace_state["completed_steps_summary"].items():
                    justification = step_data.get('justification', '未提供')
                    summary = step_data.get('summary', '无总结')
                    charts_info = ", ".join(step_data.get('generated_charts', {}).keys()) or "无"
...
{json.dumps(plan, ensure_ascii=False, indent=2)}'''
            ...
            if coder_response.status == "success":
                ...
                # a. 更新全局数据路径 (如果适用)
                if key == 'eda' and not workspace_state["global_cleaned_data_path"]:
                    ...
                    for file in files_in_step:
                        if 'clean' in file.lower() and (file.endswith('.csv') or file.endswith('.xlsx')):
                            workspace_state["global_cleaned_data_path"] = file
                            logger.info(f"共享工作区状态更新：找到清洗后的数据文件 -> {file}")
                            break
                
                # b. 归档本步骤的完整信息
                step_summary_for_context = {
                    "justification": plan.get('justification', '无'),
                    "analysis_plan": plan.get('analysis_plan', '无'),
                    "summary": coder_response.summary,
                    "code_execution_result": coder_response.code_execution_result,
                    "generated_charts": {
                        chart.get('filename'): chart.get('purpose')
                        for chart in plan.get('charts', []) if isinstance(chart, dict)
                    },
                    "generated_files": await code_interpreter.get_created_files(key)
                }
                workspace_state["completed_steps_summary"][key] = step_summary_for_context
                logger.info(f"步骤 '{key}' 的完整档案已归档到上下文。")
