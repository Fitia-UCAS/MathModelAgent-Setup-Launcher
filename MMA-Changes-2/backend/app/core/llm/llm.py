...
class LLM:
    ...
    async def chat(
        ...
        kwargs = {
            ...
            "model": self.model,
            "messages": safe_messages,            # 使用清洗后的 messages
            "stream": False,
            "top_p": top_p,
        ...
        if tools:
            ...
            # 否则不传 tool_choice，交由服务端默认策略（如 auto）
...
async def simple_chat(model: LLM, history: list) -> str:
    ...
    async def summarize_chunk(chunk_msgs):
        ...
        kwargs = {
            ...
            "model": model.model,
            "messages": sanitize_messages_for_openai([sys_prompt, user_prompt]),  # 清洗
            "stream": False,
    ...
    if total_tokens <= CONTEXT_TOKEN_HARD_LIMIT:
        kwargs = {
            "api_key": model.api_key,
            "model": model.model,
            "messages": sanitize_messages_for_openai(history),  # 清洗
            "stream": False,
            "request_timeout": REQUEST_TIMEOUT,
            "client_args": {"timeout": HTTPX_TIMEOUTS},
        }
    ...
    for round_idx in range(MAX_SUMMARY_ROUNDS):
        ...
        if exact_total <= CONTEXT_TOKEN_HARD_LIMIT:
            ...
            kwargs = {
                "api_key": model.api_key,
                "model": model.model,
                "messages": sanitize_messages_for_openai(new_history),  # 清洗
                "stream": False,
                "request_timeout": REQUEST_TIMEOUT,
                "client_args": {"timeout": HTTPX_TIMEOUTS},
            }
    ...
    kwargs = {
        "api_key": model.api_key,
        "model": model.model,
        "messages": sanitize_messages_for_openai(final_history),  # 清洗
        "stream": False,
        "request_timeout": REQUEST_TIMEOUT,
        "client_args": {"timeout": HTTPX_TIMEOUTS},
    }
