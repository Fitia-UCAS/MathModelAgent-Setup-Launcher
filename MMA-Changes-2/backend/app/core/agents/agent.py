        ...
        if "tool_calls" in msg and isinstance(msg["tool_calls"], (list, tuple)):
            try:
                ...
                for tc in msg["tool_calls"]:
                    ...
                    fn_name = getattr(fn, "name", None) if fn is not None else (fn.get("name") if isinstance(fn, dict) else None)
                    fn_args = getattr(fn, "arguments", None) if fn is not None else (fn.get("arguments") if isinstance(fn, dict) else None)
