...
class LLM:
    ...
    async def send_message(self, response, agent_name, sub_title=None):
        ...
        # 兼容 content 为 None 的情况（例如消息只包含 tool_calls）
        ...
        # 如果存在工具调用但没有文本，打个日志，方便观测与后续增强（目前不改变发布结构）
        if tool_calls and not safe_content:
            logger.info(
                f"[{agent_name}] 本次响应包含 tool_calls 但无纯文本内容，将以空文本发布。tool_calls 数量: {len(tool_calls)}"
            )

        match agent_name:
            case AgentType.CODER:
                agent_msg: CoderMessage = CoderMessage(content=safe_content)
            case AgentType.WRITER:
                # 处理 Markdown 格式的图片语法 + 脚注拆分
                safe_content, _ = split_footnotes(safe_content)
                safe_content = transform_link(self.task_id, safe_content)
                agent_msg: WriterMessage = WriterMessage(
                    content=safe_content,
                    sub_title=sub_title,
                )
            case AgentType.MODELER:
                agent_msg: ModelerMessage = ModelerMessage(content=safe_content)
            case AgentType.SYSTEM:
                agent_msg: SystemMessage = SystemMessage(content=safe_content)
            case AgentType.COORDINATOR:
                agent_msg: CoordinatorMessage = CoordinatorMessage(content=safe_content)
            case _:
...
async def simple_chat(model: LLM, history: list) -> str:
    ...
    # 这里也做个兜底，避免 simple_chat 在 content 为空时报错
