...
class IdentifierReplacer(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        ttk.Button(btns, text="应用修改", command=self.apply_changes).grid(row=0, column=2, sticky="ew", padx=4, pady=4)
        ttk.Button(btns, text="恢复 .bak", command=self.restore_backups).grid(
            row=1, column=0, sticky="ew", padx=4, pady=4
        )
        ttk.Button(btns, text="打开目录", command=self.open_dir).grid(row=1, column=1, sticky="ew", padx=4, pady=4)
